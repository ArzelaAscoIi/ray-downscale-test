...
async def main():
    ...
    for i in range(NUM_CLIENTS):
        ...
        tbars.append(
            tarange(num_requests, desc=f"Client #{i}", position=i + 1, leave=False)
        )
        tasks.append(
            loop.create_task(
                send_requests(deployment_name=DEPLOYMENT_NAME, tbar=tbars[i])
            )
        )
...
async def monitor_replicas(deployment_name):
    replicas_bar = tqdm(
        position=0,
        total=MAX_REPLICAS,
        bar_format="Deployment replicas|{bar}|{n_fmt}/{total_fmt}",
    )
    while True:
        replicas_bar.update(
            serve.get_deployment(deployment_name).num_replicas - replicas_bar.n
        )
